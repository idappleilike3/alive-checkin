...
# é€¾æ™‚æœªå ±å¹³å®‰ï¼šæœƒå“¡å¯é¸ 24ï¼48ï¼72 å°æ™‚ï¼ˆå–ä»£èˆŠå›ºå®š 36h ä¸»æµç¨‹ï¼‰
...
def normalize_grace_hours(value, default=DEFAULT_GRACE_HOURS):
    """Clampï¼å°é½Šåˆ°å…è¨±çš„ 24ï¼48ï¼72ï¼›èˆŠå€¼ï¼ˆå¦‚ 36ï¼‰å°±è¿‘å°é½Šã€‚"""
    try:
        hours = int(value)
...
def line_auto_reply_text(text, status=None):
    ...
    if any(keyword in text for keyword in FAQ_KEYWORDS):
        ...
        return (
            ...
            "Aï¼šå¯åœ¨æœƒå“¡ä¸­å¿ƒé¸ 24ï¼48ï¼72 å°æ™‚ï¼ˆé è¨­ 48ï¼‰ã€‚æ»¿è¨­å®šæ™‚æ•¸å¾Œé€šçŸ¥ï¼›å¦æœ‰ç´„ 15 åˆ†é˜å¯å–æ¶ˆé è­¦ç·©è¡ã€‚\n\n"
...
def build_status(profile, state=None, now=None):
    ...
    if not last:
        ...
        status_text = "é è­¦å–æ¶ˆæœŸï¼Œå¯ä¸€éµå–æ¶ˆ"
        status_class = "warning"
    elif overdue:
        status_text = "å·²è¶…éå¯¬é™æ™‚é–“"
        status_class = "danger"
        ...
        "bound_guardians": [
            {
                ...
                # role = æ ¸å¿ƒï¼ä¸€èˆ¬å±¤ç´šï¼›contact_role = å®ˆè­·äººï¼ç·Šæ€¥è¯çµ¡äººï¼ˆå‹¿æ··ç”¨ï¼‰
                "role": "æ ¸å¿ƒ" if c.get("is_primary") else "ä¸€èˆ¬",
...
ALREADY_BOUND_MESSAGE = "ä½ å·²ç¶“æ˜¯å®ˆè­·äººäº†ï¼Œè«‹æŠŠé‚€è«‹è½‰å‚³çµ¦å…¶ä»–å¥½å‹"
...
def invite_bind_preview(data_file, payload):
    ...
    return {
        ...
        "message": (
            f"{inviter_name} å·²æ˜¯ä½ çš„å®ˆè­·å°è±¡ï¼å·²åŠ å…¥ï¼Œæ˜¯å¦äº’ç›¸è¨­ç‚ºå®ˆè­·äººï¼Ÿ"
            if is_reverse
...
def build_bind_success_notices(inviter, contacts, inviter_id, guardian_name, *, invite_reward_applied=False):
    ...
    inviter_notice = (
        ...
        f"ç›®å‰ï¼šæ ¸å¿ƒå®ˆè­·äºº {core_n} ä½ï¼ä¸€èˆ¬å®ˆè­·äºº {general_n} ä½ã€‚\n\n"
    ...
    guardian_notice = (
        ...
        f"âš ï¸ å°æ–¹é€¾æ™‚æœªå ±å¹³å®‰ï¼ˆä¾å°æ–¹è¨­å®šçš„ 24ï¼48ï¼72 å°æ™‚ï¼‰\n"
...
def bind_emergency_contact(
    ...
    # äº’ç¶å¯é¸ï¼šå…©é‚Šäº’ç›¸è¨­ç‚ºæ ¸å¿ƒï¼ˆå„å—æ–¹æ¡ˆ core_guardian_alert_limit ç´„æŸï¼‰
    ...
    if is_reverse_invite and not was_duplicate:
        bind_message = (
            f"äº’ç¶å®Œæˆï¼ä½ èˆ‡ã€Œ{inviter_name}ã€å·²äº’ç›¸è¨­ç‚ºå®ˆè­·äººã€‚"
            + ("ï¼ˆå·²åŒæ™‚è¨­ç‚ºæ ¸å¿ƒå®ˆè­·äººï¼‰" if mutual_core_applied else "")
        )
...
def trigger_sos(data_file, payload, config=None):
    ...
    2. 5 åˆ†é˜å†·å»(profile.last_sos_at + 300 ç§’å…§æ‹’çµ•)
    3. éé‡ alert:è¨˜éŒ„ + admin æ”¶åˆ°å‘Šè­¦(ä½†ä¸ç™¼é€ SOS)
    """
    line_user_id = str(payload.get("line_user_id") or "").strip()
    if not line_user_id:
        return {"error": "missing line_user_id"}, 400
    ...
    # SOS ä¸ä¾æ–¹æ¡ˆï¼åƒ¹æ ¼åˆ†ç´šï¼šæ‰€æœ‰æœƒå“¡çš†å¯ç”¨ï¼ˆä»å—æ¯æ—¥ä¸Šé™ï¼å†·å»é˜²è­·ï¼‰
    rules = plan_rules(profile)
        ...
        if admin_id:
            try:
                ...
                sender_admin(token_admin, admin_id, (
                    f"ğŸš¨ [SOS ç•°å¸¸] ç”¨æˆ¶ {profile.get('display_name') or line_user_id} "
                    f"ä»Šæ—¥å·²æŒ‰ SOS {sos_log.get('count', 0)} æ¬¡(ä¸Šé™ {SOS_DAILY_LIMIT})ï¼Œè«‹è¯ç¹«ç¢ºèªã€‚"
                ))
            except Exception:
                pass
    ...
    # æ ¸å¿ƒå®ˆè­·äººï¼ˆis_primaryï¼‰å„ªå…ˆæ”¶åˆ° SOSï¼›å…¶é¤˜ä¾ priority
    # èˆ‡å®‰å…¨å®ˆè­·ç›¸åŒï¼šåªç”¨ contact_is_bound_guardianï¼Œæ’é™¤æœ¬äºº IDï¼ç·Šæ€¥è¯çµ¡äºº
    contacts = sorted(
        profile.get("contacts") or [],
        key=lambda item: (0 if item.get("is_primary") else 1, int(item.get("priority") or 9999)),
    )
    ...
    # å€‹äººå®ˆè­·äººæˆ–å®ˆè­·ç¾¤ä»»ä¸€å¯é€ï¼›å…©è€…éƒ½æ²’æœ‰æ‰æ‹’çµ•ï¼ˆæ–¹æ¡ˆæœ¬èº«ä¸æœƒè‡ªå‹•ç¶å®šå°è±¡ï¼‰
    ...
    if not claim.get("claimed"):
        ...
        if claim.get("reason") == "recover_pending":
            ...
            if recovered_code == 200:
                ...
                guardian_rows = [
                    {
                        "name": row.get("name") or "æ ¸å¿ƒå®ˆè­·äºº",
                        "status": row.get("status"),
    ...
    message = (
        f"ğŸš¨ã€SOS ç·Šæ€¥æ±‚åŠ©ã€‘{profile.get('display_name') or 'ä½ çš„è¦ªå‹'} ç™¼å‡ºç·Šæ€¥æ±‚åŠ©ï¼Œ\n"
        f"è«‹ç«‹å³è¯çµ¡æœ¬äººä¸¦ç¢ºèªå®‰å…¨ã€‚è‹¥æœ‰ç«‹å³å±éšªï¼Œè«‹æ’¥æ‰“ 119ã€‚{location_text}\n\n"
        "æœ¬é€šçŸ¥ä¸æœƒè‡ªå‹•è¯çµ¡è­¦æ¶ˆï¼Œè«‹ä¾ç¾å ´ç‹€æ³ä¸»å‹•æ±‚åŠ©ã€‚"
    )
    ...
    if budget.get("reason"):
        append_notification_log(
            ...
            line_user_id,
            "budget_limited",
            "SOS é¡å¤–æ¨æ’­å·²ä¾æˆæœ¬ä¸Šé™ç¸®æ¸›",
            json.dumps(budget, ensure_ascii=False),
        )

    self_confirmation = (
        "âœ… SOS é€šçŸ¥æµç¨‹å·²å®Œæˆ\n"
        "è«‹ç•™æ„ä¸‹æ–¹é€šçŸ¥æ˜ç´°ï¼›è‹¥æ˜¯èª¤è§¸æˆ–ç›®å‰å·²å®‰å…¨ï¼Œ"
        "è«‹åœ¨ 10 åˆ†é˜å…§å›åˆ° SOS ç•«é¢å–æ¶ˆé€šçŸ¥"
    )
    prepared_deliveries = [
        {
            "kind": "guardian",
            ...
            "display_name": str(
                contact.get("name") or contact.get("relationship") or "æ ¸å¿ƒå®ˆè­·äºº"
            ),
            "status": "pending",
            "retry_key": _line_retry_key(
...
ëú“Öº‡B–N‡–ú3–7¢¦›ˆ(€€€€€€€€€€€€€€€€€€€€¤(€€€€€€€€€€€€€€€¥˜É•ÍÕ±Ğ¹•Ğ ‰™…¥±•ˆ¤…¹¹½ĞÉ•ÍÕ±Ğ¹•Ğ ‰‰½Ñ}¹½Ñ}…‘µ¥¹}½Õ¹Ğˆ¤è(€€€€€€€€€€€€€€€€€€€µÍ}±¥¹•Ì¹…ÁÁ•¹¡˜‹¢®/–ë–’ÇšV\éí±•¸¡É•ÍÕ±Ñl™…¥±•t¥ôƒ’ö7ˆ¤(€€€€€€€€€€€€€€€€Œƒ––r 	½Ğƒ‡º‡B–N‡š²+¦fCrj¢â‹’êë–’ÇšV_šfš&7š>C’ë¾òo–6Òk¾ò?Ú–ºk–ú3’öÿR£¢–ŞË¢«–.Wšb¿–º#¢¶ßú“º‡B–N„(€€€€€€€€€€€€€€€¥˜É•ÍÕ±Ğ¹•Ğ ‰‰½Ñ}¹½Ñ}…‘µ¥¹}½Õ¹Ğˆ¤è(€€€€€€€€€€€€€€€€€€€µÍ}±¥¹•Ì¹…ÁÁ•¹ ‹Â~J„ƒ¢.—¦r¢®/–ë¢Ú¦†7š"C–N‡¾ò3–>¿–r£ú“¢‡š&O3º‡B–N‡¢¢·–ºk7r/šVg–¶ã¾ò#¦v{–ş¢š¦Z/¦kš¶—¦¦¾ò$ˆ¤(€€€€€€€€€€€€€€€±¥¹•}‰½Ñ}…Á¤¹ÁÕÍ¡}µ•ÍÍ…”¡É½ÕÁ}¥°Q•áÑM•¹‘5•ÍÍ…”¡Ñ•áĞô‰q¸ˆ¹©½¥¸¡µÍ}±¥¹•Ì¤¤¤(€€€€€€€€€€€•á•ÁĞá•ÁÑ¥½¸è(€€€€€€€€€€€€€€€Á…ÍÌ((€€€€€€€¥˜A½ÍÑ‰…­Ù•¹Ğ¥Ì¹½Ğ9½¹”è(€€€€€€€€€€€¡…¹‘±•È¹…‘¡A½ÍÑ‰…­Ù•¹Ğ¤(€€€€€€€€€€€‘•˜¡…¹‘±•}Á½ÍÑ‰…¬¡•Ù•¹Ğ¤è(€€€€€€€€€€€€€€€±¥¹•}ÕÍ•É}¥€ô•Ñ…ÑÑÈ¡•Ù•¹Ğ¹Í½ÕÉ”°€‰Õ