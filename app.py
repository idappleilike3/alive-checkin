Warning: truncated output (original token count: 214361)
Total output lines: 20834
...
def can_view_guardian_group_status(state, group, line_user_id) -> bool:
    ...
    ``notify_admin_only`` controls names included in pushed summaries. It must
    not block an invited guardian who actively signs in and opens the status
    page. Mere LINE group membership is never sufficient.
    """
    uid = str(line_user_id or "").strip()
    if not uid or not isinstance(group, dict):
        return False
...
def trigger_sos(data_file, payload, config=None):
    ...
    # SOS 不依方案／價格分級：…14361 tokens truncated…    for event in (state.get("account_migration_audit") or [])
        if isinstance(event, dict)
        and (
