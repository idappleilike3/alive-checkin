...
from finance_admin import (
    ...
    create_essential_service as create_finance_essential_service,
    create_expense as create_finance_expense,
    finance_dashboard,
    update_essential_service as update_finance_essential_service,
...
DEFAULT_CARD_TEMPLATE = {
    ...
    "buttons": [
        {"label": "æˆ‘å¹³å®‰", "action": "checkin"},
        {"label": "å®‰å…¨å®ˆè­·", "uri": "https://alive-checkin.onrender.com/liff/checkin.html?open=guard"},
        {"label": "éœ€è¦å¹«å¿™", "uri": "https://alive-checkin.onrender.com/liff/sos.html"},
        {"label": "æŸ¥çœ‹ä»Šæ—¥å®‰å¿ƒæé†’", "uri": "https://alive-checkin.onrender.com/daily-care.html"},
        {"label": "é‚€è«‹è¦ªå‹æˆç‚ºå®ˆè­·äºº", "uri": "https://alive-checkin.onrender.com/liff/share-invite.html"},
    ],
}
...
def line_auto_reply_text(text, status=None):
    ...
    return f"å°å¹³å®‰è‡ªå‹•å›è¦†ï¼š\n{xiao_pingan['answer']}\n\nä¹Ÿå¯ä»¥åˆ°æœƒå“¡ä¸­å¿ƒè¯çµ¡äººå·¥å®¢æœã€‚"
...
def should_create_support_ticket(text):
    ...
    if len(text) <= 5:
        return False
    explicit_handoff = ("äººå·¥", "å®¢æœ", "å°ˆäºº", "å›è¦†æˆ‘", "è¯çµ¡æˆ‘", "å•é¡Œå›å ±")
...
def mutate_state_atomically(data_file, mutator, *, skip_if_unchanged=False):
    ...
    if database_url():
        ...
        try:
            ...
            if skip_if_unchanged and row and working == state:
                conn.rollback()
                return result
            current_revision = (
                int(row[1])
...
def persisted_finance_dashboard(data_file, month, config=None):
    """Build and persist the finance dashboard under the state mutation lock."""
    clock = current_app_time(config or {})
    return mutate_state_atomically(
        data_file,
        lambda state: finance_dashboard(state, month, now=clock),
...
def build_checkin_message_context(profile, now):
    """æœ¬äººèˆ‡å®ˆè­·äººå…±ç”¨çš„ç¥ç¦ã€ç­‰ç´šåŠé‡å¤§å®‰å…¨æ¶ˆæ¯ã€‚"""
    ...
    return {
        ...
        "level_name": str(level.get("level_name") or "å®‰å¿ƒå•Ÿç¨‹").strip(),
        "safety_news": (
            str(care.get("news_summary") or "").strip()
            if care.get("has_important_news")
            else "ä»Šæ—¥æš«ç„¡é‡è¦å®‰å¿ƒæé†’"
        ),
    }


def build_checkin_success_text(status, *, now=None, config=None):
    """å ±å¹³å®‰æˆåŠŸå›è¦†ï¼šçœŸå¯¦æ™‚é–“ã€ç¥ç¦ã€ç­‰ç´šã€å®‰å…¨æ¶ˆæ¯èˆ‡ä¸‹æ¬¡æé†’ã€‚"""
    now = now or current_app_time(config)
    ...
    lines = [
        (
            "âœ… ä»Šå¤©å·²ç¶“å ±éå¹³å®‰äº†ï¼Œä¸ç”¨å†é»ä¸€æ¬¡ã€‚"
            if duplicate
            else "âœ… å ±å¹³å®‰æˆåŠŸ"
        ),
    ]
    if not duplicate:
        lines.append(f"âœ… å·²å ±å¹³å®‰æ™‚é–“ï¼š{check_dt.strftime('%Y/%m/%d %H:%M')}")
    lines.extend([
        f"ğŸ“… ä»Šå¤©æ˜¯ {format_md_weekday(now)}",
        f"ğŸ’Œ {context['blessing']}",
        f"ğŸŒ± ç•¶å‰ç­‰ç´šï¼š{context['level_name']}ï½œé€£çºŒ {context['streak_days']} å¤©",
    ])
    if context["safety_news"]:
        lines.append(f"ğŸ“° å®‰å¿ƒæé†’ï¼š{context['safety_news']}")
    ...
    lines.append("ä»Šæ—¥å·²å ±å¹³å®‰ï¼Œè«‹æ”¾å¿ƒ")
    return "\n".join(lines)


def build_guardian_checkin_text(profile, now):
    """å®ˆè­·äººå®‰å¿ƒé€šçŸ¥ï¼›åªæä¾›å®Œæˆæ™‚é–“èˆ‡ç•¶æ—¥å®‰å…¨æ–°èã€‚"""
    owner_name = str(profile.get("display_name") or "æ‚¨çš„è¦ªå‹").strip()
    context = build_checkin_message_context(profile, now)
    lines = [
        f"âœ… {owner_name} ä»Šæ—¥å·²å ±å¹³å®‰",
        f"ğŸ•’ å®Œæˆæ™‚é–“ï¼š{now.strftime('%Y/%m/%d %H:%M')}",
    ]
    if context["safety_news"]:
        lines.append(f"ğŸ“° å®‰å¿ƒæé†’ï¼š{context['safety_news']}")
    lines.append(f"ä»Šæ—¥å·²æ”¶åˆ° {owner_name} çš„å¹³å®‰ï¼Œè«‹æ”¾å¿ƒ")
    return "\n".join(lines)
...
def repair_accepted_guardian_invites(state, profile):
    ...
    for invite in state.get("guardian_invites") or []:
        ...
        if isinstance(guardian_profile, dict):
            ...
            if detail is None:
                details.append({
                    ...
                    "display_name": str(profile.get("display_name") or "é‚€è«‹äºº"),
        ...
        relationship = str(invite.get("relationship") or "å®ˆè­·äºº").strip()
        candidate = next(
            (
...
def onboarding_status_payload(data_file, line_user_id, *, allow_missing_profile=False):
    ...
    if profile:
        activation_candidates = [parse_datetime(profile.get("profile_completed_at"))]
        activation_candidates.extend(
            parse_datetime(row.get("accepted_at") or row.get("created_at"))
            for row in (profile.get("contacts") or [])
            if isinstance(row, dict) and resolve_contact_role(row) == "guardian"
        )
        activation_candidates = [value for value in activation_candidates if value]
        activation_time = (
            max(activation_candidates, key=lambda value: value.timestamp())
            if activation_candidates else None
        )
        activation = activate_pending_membership_after_setup(
            state, line_user_id, now=activation_time
        )
    if profile and (
        repaired_beta
        or repaired_binding
        or activation.get("activated")
    ...
    membership_status_text = (
        "21 å¤©å°æµ‹ï¼ˆç­‰å¾…å®Œæˆå®ˆæŠ¤ç»‘å®šåèµ·ç®—ï¼‰"
    ...
    return {
        ...
        **access,
        ...
        "membership_status_text": membership_status_text,
        ...
        "beta_activation_pending": beta_pending,
...
def acknowledge_binding_celebration(data_file, line_user_id, payload=None):
    ...
    if birthday:
        try:
            ...
            return {"ok": False, "error": "invalid_birthday", "message": "ç”Ÿæ—¥æ ¼å¼ä¸æ­£ç¢º"}, 400
        if parsed_birthday.date() > current_app_time({}).date():
            return {"ok": False, "error": "invalid_birthday", "message": "ç”Ÿæ—¥ä¸å¯æ™šæ–¼ä»Šå¤©"}, 400
...
def update_member_location(data_file, line_user_id, payload, *, source="member"):
    """Save member profile location and optional birthday without retaining an address."""
    line_user_id = str(line_user_id or "").strip()
    city = str((payload or {}).get("city") or "").strip()
    district = str((payload or {}).get("district") or "").strip()
    has_birthday = "birthday" in (payload or {})
    has_display_name = "display_name" in (payload or {})
    ...
    if display_name and len(display_name) > 40:
        return {"ok": False, "error": "invalid_display_name", "message": "ç¨±å‘¼è«‹å‹¿è¶…é 40 å€‹å­—"}, 400
    if birthday:
        try:
            parsed_birthday = datetime.strptime(birthday, "%Y-%m-%d")
        except ValueError:
    ...
    profile.pop("address", None)
    if has_birthday:
        profile["birthday"] = birthday
    if has_display_name and display_name:
...
def _restore_beta_member_profile(state, member_id, guardian_id, tombstone, clock, actor):
    ...
    profile.update({
        ...
        "display_name": str(
            ...
            or (tombstone or {}).get("display_name")
            or "LINE ä½¿ç”¨è€…"
        ),
        ...
        "contacts": [{
            ...
            "name": str(guardian.get("display_name") or "å®ˆè­·äºº"),
            "contact_role": "guardian",
            "relationship": "å®ˆè­·äºº",
            "line_user_id": guardian_id,
            "line_id": guardian_id,
            "binding_status": "accepted",
            "consent_status": "accepted",
...
def activate_pending_membership_after_setup(state, line_user_id, *, now=None):
    ...
    profile = (state.get("users") or {}).get(member_id)
    if not isinstance(profile, dict):
        return {"activated": False, "reason": "member_not_found"}
    if not profile.get("onboarding_reminder_configured"):
        return {"activated": False, "reason": "profile_incomplete"}
    if not profile_has_bound_line_guardian(profile):
        return {"activated": False, "reason": "guardian_not_bound"}
    clock = now or current_app_time({})
    if (
        profile.get("membership_source") == "beta"
...
def send_beta_daily_feedback(config, now=None):
    ...
    for profile in (state.get("users") or {}).values():
        ...
        has_accepted_guardian = any(
            resolve_contact_role(contact) == "guardian"
            and contact_is_bound_guardian(contact, target)
            for contact in (profile.get("contacts") or [])
        )
        if (
            not target
...
def membership_period_summary(profile, *, now=None):
    """Return the authoritative trial or beta dates shown in member center."""
    source = str((profile or {}).get("membership_source") or "").strip()
    if source == "beta":
        started = parse_datetime((profile or {}).get("beta_started_at"))
        ends = parse_datetime((profile or {}).get("beta_ends_at"))
    else:
        started = parse_datetime((profile or {}).get("trial_started_at"))
        ends = parse_datetime((profile or {}).get("trial_end"))
    if not started or not ends:
        return {
            "started_date": "",
            "ends_date": "",
    ...
    if comparable_clock >= comparable_ends:
        ...
        remaining_days = 0
    elif comparable_clock < comparable_started:
        current_day = 0
        remaining_days = total_days
    else:
        current_day = min(
            total_days,
            int((comparable_clock - comparable_started).total_seconds() // 86400) + 1,
        )
        remaining_days = max(0, total_days - current_day)
    started_date = started.strftime("%Y-%m-%d")
    ends_date = ends.strftime("%Y-%m-%d")
    return {
        ...
        "display_text": (
            f"å¼€å§‹ {started_date}ï½œåˆ°æœŸ {ends_date}ï½œ"
            f"ç›®å‰ç¬¬ {current_day} å¤©ï½œå‰©ä½™ {remaining_days} å¤©"
        ),
    }
...
def register_line_user(data_file, payload):
    ...
    user = get_profile(
        state,
        line_user_id,
        # Opening or logging in only records the member. The entitlement clock
        # begins after reminder setup and an accepted guardian binding.
        start_public_trial=False,
    )
    ...
    status["membership_activation_pending"] = bool(
        not user.get("trial_started_at")
        and not str(user.get("plan") or "").startswith("paid_")
    ) or bool(user.get("beta_activation_pending"))
    return status, 200
    ...
    def mutate(state):
        ...
        for key, profile in list(users.items()):
            ...
            if key in retired or target in retired:
                cohort = str((profile or {}).get("beta_cohort") or "").upper()
                if (
                    str((profile or {}).get("membership_source") or "") == "beta"
                    and cohort in BETA_COHORT_PLAN
                ):
                    continue
                users.pop(key, None)
...
def validate_contact_payload(contact, existing=None, contact_limit=10):
    ...
    # phone format: æ¥å—å°ç£æ‰‹æ©Ÿèˆ‡å¸‚è©±ï¼›ç·Šæ€¥è¯çµ¡äººä¸æ‡‰è¢«é™åˆ¶åªèƒ½å¡«æ‰‹æ©Ÿã€‚
    if phone:
...
def invite_bind_preview(data_file, payload):
    ...
    inviter = users.get(inviter_id) if isinstance(users.get(inviter_id), dict) else {}
    invitee = users.get(invitee_id) if isinstance(users.get(invitee_id), dict) else {}
    existing_invitee_contact = next(
        (
            row for row in (inviter.get("contacts") or [])
            if isinstance(row, dict) and get_contact_line_id(row) == invitee_id
        ),
        {},
    )
    invitee_name = str(invitee.get("display_name") or "").strip()
    invitee_phone = str(invitee.get("phone") or invitee.get("mobile") or "").strip()
    if not invitee_phone:
        for owner in users.values():
            ...
            previous = next(
                (
                    row for row in (owner.get("contacts") or [])
                    if isinstance(row, dict)
                    and get_contact_line_id(row) == invitee_id
                    and str(row.get("phone") or "").strip()
                ),
                None,
            )
            if previous:
                invitee_phone = str(previous.get("phone") or "").strip()
                break
    already_bound = bool(
        existing_invitee_contact
        and str(
            existing_invitee_contact.get("consent_status")
            or existing_invitee_contact.get("binding_status")
            or ""
        ).strip() == "accepted"
    )
    ...
    pending = None
    if inviter_invites and not already_bound:
    ...
    return {
        ...
        "binding_completed": already_bound,
        "existing_relationship": str(existing_invitee_contact.get("relationship") or "").strip(),
        "invitee_profile": {
            "name": invitee_name,
            ...
            "profile_completed": bool(
                ...
                and (
                    invitee_phone
                    or invitee.get("profile_completed_at")
                    or invitee.get("onboarding_reminder_configured")
                )
            ),
        },
        ...
        "invite_status": "accepted" if already_bound else (pending or {}).get("status") or "legacy",
...
def bind_emergency_contact(
    ...
    if pending_invite and "activate_trial" in payload and (
        ...
        return {
            "ok": False,
            "error": "è«‹å¡«å¯«æœ¬äººå§“åã€èˆ‡é‚€è«‹äººçš„é—œä¿‚åŠè¯çµ¡é›»è©±å¾Œå†å®Œæˆç¶å®š",
    ...
    if contact_phone:
        contact_user["phone"] = contact_phone
    if contact_display_name and contact_phone:
        contact_user.setdefault("profile_completed_at", datetime.now().isoformat(timespec="seconds"))
    ...
    def _apply_line_bind_fields(row, *, is_new_accept):
        ...
        row["relationship"] = contact_relationship or row.get("relationship") or "å®ˆè­·äºº"
        row["phone"] = contact_phone or row.get("phone") or ""
    ...
    ensure_onboarding_completed_flag(inviter)
    activate_pending_membership_after_setup(
        state,
        inviter_id,
        now=parse_datetime(accepted_at) or current_app_time({}),
    )
    ...
    if not was_duplicate:
                ...
                rows.append(
                    {
                        ...
                        "peer_display_name": peer_name or "è¦ªå‹",
                        "role": role,
                        "accepted_at": accepted_at,
                        "acknowledged_at": "",
                    }
        ...
        queue_binding_celebration(
            ...
            peer_name=contact_display_name or "å®ˆè­·äºº",
            role="member",
        )
        queue_binding_celebration(
            contact_user,
...
def guardian_group_settings_for_user(data_file, line_user_id):
    ...
    if group_limit <= 0:
        return {
            ...
            "message": "å®ˆè­·ç¾¤æ˜¯ 799 å®¶åº­å®ˆè­·æ–¹æ¡ˆå°ˆå±¬åŠŸèƒ½ï¼Œå¦‚æœ‰éœ€è¦è«‹å…ˆå‡ç´š 799 æ–¹æ¡ˆã€‚",
