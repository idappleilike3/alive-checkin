...
def build_expiry_remind_flex(profile, now=None):
    ...
    if isinstance(days, int) and days < 0:
        ...
        countdown = "還有 4 天"
        body = (
            f"將在 4 天後到期。{action_word}後可保留您的連續報到天數與守護設定。"
        )
    elif days == 7:
        title = f"您的「{label}」即將到期"
        countdown = "還有 8 天"
        ...
        title = f"您的「{label}」即將到期"
        countdown = f"還有 {max(1, int(days or 0) + 1)} 天"
        body = (
        ...
        "contents": {
            ...
            "body": {
                ...
                "contents": [
                    ...
                    {
                        "type": "text",
                        "text": "💬 您的每一個感受都很重要，也歡迎分享使用感受與建議，讓我們做得更貼心。",
                        "size": "sm",
                        "color": "#64748B",
                        "wrap": True,
                    },
...
def handle_expiry_opt_out_postback(data_file, line_user_id):
    ...
    return (
        "💛 謝謝您告訴我們，之後不會再提醒方案到期。\n\n"
        "原有的平安紀錄與守護設定會依服務規則保留；如果之後想繼續使用，"
        "隨時回來查看方案就可以。也謝謝您曾讓每日平安陪伴您與家人。"
    )
...
def update_member_location(data_file, line_user_id, payload, *, source="member"):
    ...
    if display_name and len(display_name) > 40:
        return {"ok": False, "error": "invalid_display_name", "message": "稱呼請勿超過 40 個字"}, 400
    if birthday:
        try:
...
def validate_contact_payload(contact, existing=None, contact_limit=10):
    ...
    # phone format: 接受台灣手機與市話；緊急聯絡人不應被限制只能填手機。
    if phone:
...
def add_single_contact(data_file, line_user_id, contact_payload):
    ...
    # 第一位守護人自動成為主要守護人；之後只有會員明確指定時才替換。
    # 緊急聯絡人是獨立分類，不得因調整主要守護人而被取消標記。
...
def bind_emergency_contact(
    ...
    if pending_invite and "activate_trial" in payload and (
        ...
        return {
            "ok": False,
            "error": "請填寫本人姓名、與邀請人的關係及聯絡電話後再完成綁定",
    ...
    def _apply_line_bind_fields(row, *, is_new_accept):
        ...
        row["relationship"] = contact_relationship or row.get("relationship") or "守護人"
...
ADMIN_ROLE_PERMISSIONS = {
    ...
    "finance": {"order.manage", "finance.manage"},
    "viewer": set(),
}
...
def send_missing_contact_reminders(config):
    ...
    for user in state.get("users", {}).values():
        ...
        if user.get("membership_paused") or not membership_access_active(user, now):
            skipped += 1
            continue
        if profile_has_bound_line_guardian(user):
            skipped += 1
            continue
        if not user.get("guardian_unbound_since"):
            user["guardian_unbound_since"] = (
                user.get("beta_started_at")
                or user.get("trial_started_at")
                or user.get("membership_started_at")
                or user.get("paid_at")
                or user.get("created_at")
                or now.isoformat(timespec="seconds")
            )
        if not guardian_binding_reminder_due(user, now):
            skipped += 1
            continue
        sent_dates = set(user.get("contact_reminder_sent_dates") or [])
        if today in sent_dates:
            continue
        # 未完成 LINE 綁定時，不論是否已有手動資料、待接受邀請或電話
        # 緊急聯絡人，一律提供可直接開啟 LINE 好友分享的一鍵邀請卡。
...
def build_daily_checkin_flex(now, target_time="", profile=None):
    ...
    emergency_contact_set = any(
        resolve_contact_role(contact) == "emergency"
        and bool(str(contact.get("phone") or "").strip())
        for contact in (care_profile.get("contacts") or [])
        if isinstance(contact, dict)
    )
    if not emergency_contact_set:
        body_contents.append(
            {
                "type": "text",
...
def send_checkin_reminders(config):
    ...
    for user in state.get("users", {}).values():
        ...
        if user.get("membership_paused") or not membership_access_active(user, now):
            skipped += 1
            continue
        if not profile_has_bound_line_guardian(user):
            skipped += 1
            continue
...
def _eligible_personalized_checkin_members(state, now=None):
    """Return active, pushable members for the admin card preview/send flow."""
    current = now or current_app_time({})
    rows = []
    for key, profile in (state.get("users") or {}).items():
        if not isinstance(profile, dict):
            continue
        uid = str(profile.get("line_user_id") or key or "").strip()
        if not re.fullmatch(r"U[0-9a-fA-F]{32}", uid):
            continue
        if profile.get("line_push_blocked") or profile.get("membership_paused"):
            continue
        if not membership_access_active(profile, current):
            continue
        rows.append((uid, profile))
    return sorted(rows, key=lambda row: (str(row[1].get("display_name") or ""), row[0]))


def personalized_checkin_push_preview(data_file, now=None):
    rows = _eligible_personalized_checkin_members(load_state(data_file), now=now)
    return {
        "eligible_count": len(rows),
        "members": [
            {
                "display_name": str(profile.get("display_name") or "LINE 使用者"),
                "line_user_id": uid,
                "masked_line_user_id": _mask_line_user_id(uid),
                "plan": str(profile.get("plan") or ""),
            }
...
def admin_send_personalized_checkin_cards(
    ...
    if confirmed is not True:
        return {"ok": False, "error": "confirmation_required"}, 409
    ...
    if mode == "single":
        wanted = str(line_user_id or "").strip()
        eligible = [row for row in eligible if row[0] == wanted]
        if not eligible:
            return {"ok": False, "error": "member_not_eligible"}, 404
    sender = config.get("LINE_PUSH_SENDER") or line_push_message
    sent = failed = 0
    results = []
    for uid, profile in eligible:
        ...
        try:
            ...
            append_notification_log(state, "admin_personalized_checkin", uid, "failed", message, str(exc))
            failed += 1
            results.append({"line_user_id": uid, "status": "failed"})
    save_state(data_file, state)
    return {"ok": failed == 0, "mode": mode, "sent": sent, "failed": failed, "results": results}, 200
...
def create_app(config=None):
    ...
    app.config.update(
        ...
        SECRETS_SCAN_PASSED=os.environ.get("SECRETS_SCAN_PASSED", ""),
        DATABASE_LEAST_PRIVILEGE_CONFIRMED=os.environ.get("DATABASE_LEAST_PRIVILEGE_CONFIRMED", ""),
        DEPENDENCY_AUDIT_PASSED=os.environ.get("DEPENDENCY_AUDIT_PASSED", ""),
        BACKUP_RESTORE_TESTED_AT=os.environ.get("BACKUP_RESTORE_TESTED_AT", ""),
        SECURITY_MONITORING_ENABLED=os.environ.get("SECURITY_MONITORING_ENABLED", ""),
        INCIDENT_RUNBOOK_CONFIRMED=os.environ.get("INCIDENT_RUNBOOK_CONFIRMED", ""),
    )
    for security_check_number in range(1, 11):
        security_prefix = f"SECURITY_CHECK_{security_check_number:02d}"
        for security_field in ("STATUS", "SOURCE", "CHECKED_AT", "EVIDENCE"):
            security_key = f"{security_prefix}_{security_field}"
    ...
    @app.after_request
    def _apply_global_security_headers(response):
        return apply_security_headers(
            response,
            is_https=_admin_login_transport_secure(),
            path=request.path,
        )
    ...
    @app.get("/api/admin/personalized-checkin-push/preview")
    def admin_personalized_checkin_push_preview_api():
        denied = _admin_guard()
        if denied:
            return denied
    ...
    def admin_personalized_checkin_push_send_api():
        ...
        if denied:
            return denied
        payload = request.get_json(silent=True) or {}
        data, code = admin_send_personalized_checkin_cards(
            app.config,
        ...
        append_admin_audit(
            app.config["DATA_FILE"],
            "personalized_checkin_push.send",
            "success" if code < 400 else "failed",
            {"mode": payload.get("mode"), "sent": data.get("sent", 0), "failed": data.get("failed", 0)},
        )
        return jsonify(data), code
    ...
    @app.get("/api/admin/security-readiness")
    def admin_security_readiness_api():
        denied = _admin_guard(permission="system.manage")
        if denied:
            return denied
        return jsonify(security_readiness(app.config))

    @app.get("/api/admin/finance/dashboard")
    def admin_finance_dashboard_api():
        denied = _admin_guard(permission="finance.manage")
        if denied:
            return denied
    ...
    @app.post("/api/admin/finance/expenses")
    def admin_finance_expense_create_api():
        denied = _admin_guard(write=True, permission="finance.manage")
        if denied:
            return denied
        payload = request.get_json(silent=True)
        if not isinstance(payload, dict):
            return jsonify({"error": "invalid_finance_request", "message": "請提供正確的支出資料"}), 400
        try:
            expense = mutate_state_atomically(
                app.config["DATA_FILE"],
                lambda state: create_finance_expense(
                    state, payload, str(session.get("admin_role") or "unknown")
                ),
            )
        except FinanceValidationError as exc:
            return jsonify({"error": "invalid_finance_request", "message": str(exc)}), 400
    ...
    def admin_finance_settings_update_api():
        denied = _admin_guard(write=True, permission="finance.manage")
        if denied:
            return denied
        payload = request.get_json(silent=True)
        if not isinstance(payload, dict):
            return jsonify({"error": "invalid_finance_request", "message": "請提供正確的設定資料"}), 400
        try:
            settings = mutate_state_atomically(
                app.config["DATA_FILE"],
                lambda state: update_finance_settings(
                    state, payload, str(session.get("admin_role") or "unknown")
                ),
            )
        except FinanceValidationError as exc:
    ...
    @app.post("/api/admin/finance/services")
    def admin_finance_service_create_api():
        denied = _admin_guard(write=True, permission="finance.manage")
        if denied:
            return denied
        payload = request.get_json(silent=True)
        if not isinstance(payload, dict):
            return jsonify({"error": "invalid_finance_request", "message": "請提供正確的必要服務資料"}), 400
        try:
            service = mutate_state_atomically(
                app.config["DATA_FILE"],
                lambda state: create_finance_essential_service(
                    state,
                    payload,
                    str(session.get("admin_role") or "unknown"),
                    now=current_app_time(app.config),
                ),
            )
        except FinanceValidationError as exc:
            return jsonify({"error": "invalid_finance_request", "message": str(exc)}), 400
        return jsonify({"ok": True, "service": service}), 201

    @app.put("/api/admin/finance/services/<service_id>")
    def admin_finance_service_update_api(service_id):
        denied = _admin_guard(write=True, permission="finance.manage")
        if denied:
            return denied
        payload = request.get_json(silent=True)
        ...
        try:
            service = mutate_state_atomically(
                app.config["DATA_FILE"],
                lambda state: update_finance_essential_service(
                    state,
                    service_id,
                    payload,
                    str(session.get("admin_role") or "unknown"),
