...
部署分支為 `codex/push-management-center`。正式站必須合併到 `main` 後，才由 Render 部署合併後的 commit；本機測試使用 Python 3.14，正式建置仍以 `render.yaml` 鎖定的 Python 3.11.9 為準。

### 上線順序

1. 先在 Render／資料庫層建立可回復的完整備份，記下備份時間、目前正式 deploy ID、`main` commit 與資料 revision。不要下載或記錄任何 LINE、管理員、金流密鑰。
2. 確認 GitHub PR 的來源分支、目標 `main` 與最終 commit；所有必要檢查通過後再合併。
3. 確認 Render Web Service 與 Cron 都使用 Python 3.11，依 `requirements.txt` 完成建置；環境變數與持久磁碟設定不得清空或重建。
4. 部署完成後先檢查 `/health`。第一次讀取舊 state 時，hydration 只新增 `push_campaigns`、`push_campaign_versions`、`push_delivery_records`、`push_campaign_events` 空集合，不會改寫會員、訂單、簽到、舊通知紀錄或 revision；重跑結果必須相同。
5. 以一般後台角色登入，確認可以讀取推播清單、版本與永久發送紀錄，但看不到新增、修改、準備排程、排程及取消控制。
6. 以最高管理員建立一筆只含白名單測試 LINE UID 的草稿。先確認儲存後沒有 LINE 訊息，再修改一次，確認版本 1、版本 2 都存在，舊版不能覆蓋或刪除。
7. 按「準備排程」，確認預估人數；排程到數分鐘後。第一次正式 smoke test 禁止勾選全體方案受眾，只能使用事先核准的單一測試會員 LINE UID。
8. 到預定時間後確認 Cron 已執行、會員收到一次訊息，活動狀態正確，永久紀錄包含會員姓名、完整 LINE UID、發送當下方案、預定／實際時間、嘗試次數與成功結果。
9. 另外用不會實際送出的測試資料驗證失敗顯示：必須有中文原因、處理建議與可展開的技術訊息。完成後保留所有版本與發送紀錄，不做清除。

### 異常與回復

1. 若 build、health、權限或排程驗收失敗，立即停止新增活動，不要擴大受眾。
2. 在 Render 回復到上一次成功 deploy，記下失敗 deploy ID、commit、時間與錯誤類別；環境變數、持久磁碟與資料庫不可重建。
3. 程式碼回復只能停用新版讀寫流程，**不得刪除或截短已寫入的 `push_delivery_records`、版本或活動事件**。回復後產生的系統通知也必須繼續保留原始資料，等待新版程式再次上線讀取。
4. 若資料結構異常，先從部署前備份複製到隔離環境分析；未確認永久紀錄完整前，不得覆蓋正式資料庫。
5. 修正後重新走「一般角色唯讀 → 最高管理員草稿 → 新版本 → 白名單排程 → 永久紀錄」完整 smoke test，才可恢復正常使用。
