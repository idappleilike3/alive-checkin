...
class Release20260729Tests(unittest.TestCase):
    ...
    def test_five_pages_and_native_controls_are_present(self):
        ...
        self.assertIn("14 天免費體驗｜199 平安版", trial)
        self.assertIn("199 平安版｜月費", trial)
        self.assertIn("199 平安版｜年費", trial)
        self.assertIn("查看所有方案比較", trial)
        self.assertIn("個人安心練習｜21天體驗", beta)
        self.assertIn("家庭守護練習｜21天體驗", beta)
        self.assertNotIn("399 年費安心版｜21 天封測", beta)
        self.assertNotIn("799 年費守護版｜21 天封測", beta)
        self.assertIn(">查看方案</a>", beta)
        self.assertIn("一鍵分享邀請核心守護人", share)
        self.assertIn("成為守護人免費，不會自動購買方案", share)
        self.assertIn("立即選擇 LINE 好友分享", share)
        self.assertNotIn("第一步：加入「每日平安」官方 LINE", share)
        self.assertIn("14 天免費體驗｜199 平安版", invite)
        self.assertIn("查看 14 天免費體驗完整內容", invite)
    ...
    def test_beta_199_and_member_collapsible_history_support_and_calendar(self):
        ...
        beta = (ROOT / "beta-register.html").read_text(encoding="utf-8")
        admin = (ROOT / "admin.html").read_text(encoding="utf-8")
        self.assertNotIn("199 平安版｜月費", beta)
        self.assertNotIn("199 平安版｜年費", beta)
        self.assertIn('class="pricing-link"', beta)
