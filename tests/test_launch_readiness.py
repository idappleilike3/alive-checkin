...
class LaunchReadinessTests(unittest.TestCase):
    ...
    def test_second_stage_can_open_all_thirty_slots_after_gate(self):
        state = {
        ...
        self.assertTrue(app_module.beta_release_allowed(state, 30, now=self.now)[0])
        self.assertEqual(
            app_module.beta_release_allowed(state, 31, now=self.now),
            (False, "second_stage_max_30"),
        )

    def test_admin_assignment_requires_ten_mature_a_members(self):
        state = {
            "users": {
                **{
                    f"U-a-{index}": {
                        "line_user_id": f"U-a-{index}",
    ...
    def test_admin_assignment_blocks_unready_or_immature_second_batch(self):
        ...
        for index in range(10):
            base["users"][f"U-a-{index}"]["beta_started_at"] = (
                self.now - timedelta(days=7)
            ).isoformat(timespec="seconds")
