...
class BetaCohortPolicyTests(unittest.TestCase):
    ...
    def test_second_stage_requires_all_10_a_members_to_reach_day_7(self):
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(ValueError, "a_cohort_incomplete"):
                app_module.assign_beta_member_with_release_gate(
                    self.state, "U-20", "B399", now=self.now
                )

    def test_day_7_readiness_opens_fixed_20_399_and_10_family_slots(self):
        ...
        with patch.object(
            ...
            for index in range(11, 31):
                app_module.assign_beta_member_with_release_gate(
                    self.state, f"U-{index}", "B399", now=self.now
                )
            for index in range(31, 41):
                app_module.assign_beta_member_with_release_gate(
                    self.state, f"U-{index}", "B799", now=self.now
                )

        snapshot = app_module.beta_members_snapshot(self.state, self.now)
        self.assertEqual(snapshot["counts"], {"A": 10, "B399": 20, "B799": 10})

    def test_beta_snapshot_reports_day_1_7_14_21_milestones(self):
        app_module.assign_beta_cohort(
            self.state, "U-1", "A", now=self.now - timedelta(days=14)
        )

        member = app_module.beta_members_snapshot(self.state, self.now)["members"][0]

        self.assertEqual(member["current_day"], 15)
        self.assertEqual(
            member["milestones"],
            {"day_1": True, "day_7": True, "day_14": True, "day_21": False},
        )
